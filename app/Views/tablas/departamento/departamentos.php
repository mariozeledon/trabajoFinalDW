...
<a href="<?=base_url('formAgregarDepartamento')?>">
...
<?php
    $no_fila=0;
    foreach($misDepartamentos as $dato):
        $no_fila = $no_fila + 1; 
        if($no_fila%2==0){
    ...
    ?>
    <td><?=$dato['cod_depto']; ?></td>
    <td><?=$dato['nombre']; ?></td>
    <td><?=$dato['cod_region']; ?></td>
    <td>
        <a href="<?=base_url('formActualizarDepto/'.$dato['cod_depto'])?>">
        <span class="material-icons">edit</span>Actualizar 
        </a>

        <a href="<?=base_url('eliminarDepto/'.$dato['cod_depto'])?>">
